...
static inline void sha256(const u8 *data, unsigned int len, u8 *output)
{
	struct sha256_state sctx;

	sha256_init(&sctx);
	sha256_update(&sctx, data, len);
	sha256_final(&sctx, output);
}
...
void mptcp_crypto_key_sha(u64 key, u32 *token, u64 *idsn)
{
	...
	sha256((__force u8 *)&input, sizeof(input),
	       (u8 *)mptcp_hashed_key);
...
void mptcp_crypto_hmac_sha(u64 key1, u64 key2, u8 *msg, int len, void *hmac)
{
	 ...
	 */
	sha256((__force u8 *)&input, SHA256_BLOCK_SIZE + len,
